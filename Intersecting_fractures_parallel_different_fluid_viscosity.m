...
visN=[0.001,0.005,0.010,0.050]*10^(-9);%viscosity of fluid, Gpa*s
Cv=length(visN);

cona=4*10^(-10);%fracture connectivity,dimensionless
...
vis=visN(CI);%fracture thickness
con=cona*c/vis;
...
v(FN,CI)=(2*pi*ft)/real(keff);
inQ(FN,CI)=2*imag(keff)/real(keff);
...
figure(1)
semilogx(f,v(:,1),'r','linewidth',1.3);
hold on;
semilogx(f,v(:,2),'g','linewidth',1.3);
hold on;
semilogx(f,v(:,3),'b','linewidth',1.3);
hold on;
semilogx(f,v(:,4),'k','linewidth',1.3);
% hold on;
% semilogx(f,v(:,5),'c','linewidth',1.3);
% hold on;
% semilogx(f,v(:,6),'m','linewidth',1.3);
% hold on;
% semilogx(f,v(:,7),'c','linewidth',1.3);

xlabel('Frequency(Hz)','FontSize',12);
ylabel('{\itv_{p}}(m/s)','FontSize',12);
legend('{\it\eta} = 0.001 Pa*s','{\it\eta} = 0.005 Pa*s','{\it\eta} = 0.010 Pa*s','{\it\eta} = 0.050 Pa*s');
xlim([0.001 100000]);

figure(2)
loglog(f,inQ(:,1),'r','linewidth',1.3);
hold on;
loglog(f,inQ(:,2),'g','linewidth',1.3);
hold on;
loglog(f,inQ(:,3),'b','linewidth',1.3);
hold on;
loglog(f,inQ(:,4),'k','linewidth',1.3);
% hold on;
% semilogx(f,inQ(:,5),'c','linewidth',1.3);
% hold on;
% semilogx(f,inQ(:,6),'m','linewidth',1.3);
% hold on;
% semilogx(f,inQ(:,7),'c','linewidth',1.3);
